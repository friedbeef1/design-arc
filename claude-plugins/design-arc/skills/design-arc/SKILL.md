...
Google Stitch is valuable for canvas-based editing, multiple visual alternatives, and sustained visual refinement. Claude Code can prepare a lightweight static journey board, while Stitch supplies the polished editable-canvas route. Stitch remains optional and separately authorized.

Immediately after the Direction Gate resolves and before either renderer starts, present the visualization choice once. Do not wait for Claude Code to struggle, for a later trigger, or for a first render. Explain the differences briefly and present:

1. **Both Claude Code and Stitch — recommended**
2. **Claude Code only**
3. **Stitch only**

Reply with `1`, `2`, or `3`. Both means create the Claude Code board and the Stitch visual workspace concurrently from the same approved journey. Claude Code only is the lightweight static-board route. Stitch only is the persistent editable-canvas route. The active host remains the evidence, validation, and approval surface; inspect every selected output before assigning the visual verdict.
