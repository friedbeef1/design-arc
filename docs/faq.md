...
| **Do I need to find and install a Stitch MCP for Codex?** | No. The Live Codex plugin includes the Google-hosted Stitch connection definition and prompts for your own key when needed. It includes neither the key nor Google server code. Claude Code and Antigravity automated onboarding remain Alpha future work. | [Design Arc for Codex](codex.md#guided-stitch-connection) |
