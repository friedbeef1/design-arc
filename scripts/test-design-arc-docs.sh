...
require_text "$codex_edition" '## Guided external connections'
require_text "$codex_edition" 'GitHub marketplace installs include the Stitch connection definition. OpenAI Plugin Directory installs currently omit it'
require_text "$codex_edition" 'https://stitch.googleapis.com/mcp'
require_text "$codex_edition" 'Never paste a Stitch API key into chat.'
require_text "$codex_edition" 'Profile Picture → **Stitch settings** → **API key** → **Create key**'
require_text "$codex_edition" 'Connect Stitch for Design Arc.'
require_text "$codex_edition" 'https://mobbin.com/signup'
require_text "$codex_edition" 'https://mobbin.com/pricing'
require_text "$codex_edition" 'Mobbin lets Design Arc inspect complete real-product journeys—not merely isolated screenshots.'
require_text "$codex_edition" 'Pro is recommended for an individual'
require_text "$codex_edition" 'Team is for multiple collaborators and includes Pro capabilities'
require_text "$getting_started" '| Guidelines only | No paid external research service required |'
require_text "$getting_started" '| Guidelines + Benchmarks | Mobbin Pro recommended for an individual |'
require_text "$getting_started" '| Codex screens | No Stitch connection required |'
require_text "$getting_started" '| Stitch or Both | Google Stitch account and API key |'
require_text "$getting_started" 'External pricing, availability, and quotas may change.'
require_text "$claude_edition" 'Automated Stitch detection, setup, and connection verification are not implemented in the Claude Code Alpha adapter yet.'
require_text "$antigravity_edition" 'Automated Stitch detection, setup, and connection verification are not implemented in the Antigravity Alpha adapter yet.'
require_text "$antigravity_edition" 'search for **Stitch**, install it, enter the Stitch API key in the store flow, and ask Antigravity to “List my Stitch projects.”'
require_text "$antigravity_edition" 'https://codelabs.developers.google.com/design-to-code-with-antigravity-stitch'
...
| --- | --- |
| [![Design Arc product walkthrough thumbnail — opens YouTube](https://img.youtube.com/vi/bbc1t_1LdWk/maxresdefault.jpg)](https://youtu.be/bbc1t_1LdWk) | [![Design Arc introduction deck cover — opens presentation](https://docs.google.com/presentation/d/1EG5vrPC5UqAkNAr9jce0lvZz1Jq5oI4AqAbPVivG2sU/export/png?id=1EG5vrPC5UqAkNAr9jce0lvZz1Jq5oI4AqAbPVivG2sU&pageid=g3fab99837d7_0_185)](https://docs.google.com/presentation/d/1EG5vrPC5UqAkNAr9jce0lvZz1Jq5oI4AqAbPVivG2sU/preview) |
| [Watch on YouTube](https://youtu.be/bbc1t_1LdWk) | [Open presentation](https://docs.google.com/presentation/d/1EG5vrPC5UqAkNAr9jce0lvZz1Jq5oI4AqAbPVivG2sU/preview) |"""
...
require_text "$using_design_arc" 'After you approve the direction, Design Arc asks how to visualize it before generating anything.'
require_text "$using_design_arc" 'It does not build disposable application logic merely to visualize the proposal.'
require_text "$using_design_arc" 'The upfront choices are **Both the AI coding platform and Stitch — recommended**, **AI coding platform only**, or **Stitch only**.'
require_text "$using_design_arc" 'Design Arc never waits for the platform board to be inadequate before mentioning Stitch.'
require_text "$using_design_arc" 'When you choose both, Design Arc starts the AI coding platform board and Stitch workspace concurrently from the same approved specification.'
require_text "$using_design_arc" 'It waits for both initial renders before correcting either one.'
require_text "$using_design_arc" 'Each render is then checked independently against the approved specification before the pair is compared.'
require_text "$using_design_arc" 'After that comparison, you choose whether corrections continue on the platform version, the Stitch version, or both.'
require_text "$using_design_arc" 'Choosing both initially does not commit you to correcting both.'
require_text "$using_design_arc" 'When only one is selected, Design Arc changes only that version and preserves the other as comparison evidence.'
require_text "$using_design_arc" 'When both are selected for a correction round, those corrections run concurrently.'
require_text "$using_design_arc" 'Both share one limit of three proposal-wide correction rounds; they do not receive three rounds each.'
require_text "$faq" '**What happens when I choose Both?**'
require_text "$faq" 'The initial platform board and Stitch workspace start concurrently from the same approved specification. After comparing them independently, you choose whether to refine the platform version, the Stitch version, or both.'
...
require_text "$using_design_arc" 'GitHub marketplace installs of the Live Codex plugin include the Google-hosted Stitch MCP connection definition'
require_text "$using_design_arc" 'OpenAI Plugin Directory uploads currently omit `mcpServers`'
require_text "$using_design_arc" 'Google now provides an official Stitch MCP server and SDK'
require_text "$using_design_arc" 'Neither route bundles Google server code, a Stitch account, or credentials.'
...
require_text "$evidence_methodology" 'Immediately after direction approval and before rendering, Design Arc offers three choices'
require_text "$evidence_methodology" 'Design Arc recommends connecting now'
