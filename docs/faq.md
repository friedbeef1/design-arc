...
| **What happens when I choose Both?** | The platform board and Stitch workspace start concurrently from the same approved specification. Design Arc waits for both, checks each independently, and uses one shared three-round correction limit. | [Choosing the visual workspace](using-design-arc.md#choosing-your-ai-coding-platform-or-stitch-for-the-screens) |
