...
Treat `Both` as one paired proposal with one shared correction-round counter: at most three proposal-wide correction rounds total, not three rounds per renderer. For each paired correction round, send renderer-specific correction batches concurrently to every renderer that has repairable drift, wait for every requested correction render to finish, then reinspect each renderer independently before comparing the pair. A renderer that does not need a correction keeps its most recently inspected proposal and does not consume a separate round.
