...
GitHub marketplace installs of the Live Codex plugin include the Google-hosted Stitch MCP connection definition; OpenAI Plugin Directory uploads currently omit `mcpServers`, so Design Arc checks the active installation and guides Codex configuration when needed. Neither route bundles Google server code, a Stitch account, or credentials. Stitch remains separately authorized through the user's own API key. The Claude Code and Antigravity Alpha adapters do not yet include automated Stitch onboarding. Google now provides an official Stitch MCP server and SDK. When a particular review actually uses an MCP, its run record and evidence labels name the exact configured MCP server or tool; otherwise they name the browser or manual access path used. Design Arc does not imply an official Mobbin MCP integration.
